    ...
    try {
        ...
        paypal.payment.create(create_payment_json, async (error, paymentInfo) => {
            if (error) {
                ...
                res.status(200).json({ success: true, message: 'Order created successfully', approvalUrl, orderId: newlyCreatedOrder._id });
            }
        })
    } catch (e) {
        console.log(e)
...
const capturePayment = async (req, res) => {
    try {
        ...
        for(let item of order.cartItems){
            ...
            if(!product){
                return res.status(404).json({ success: false, message: `Not enough stock for ${product.title}` });
            }
