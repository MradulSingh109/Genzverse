...
const addAddress = async (req, res) => {
    try {
        ...
        if (!userId || !address || !city || !state || !pincode || !phone || !notes) {
            return res.status(400).json({ success: false, message: 'Invalid Data provided!' });
        }

        const newlyCreatedAddress = new Address({ userId, address, city, state, pincode, phone, notes });
...
const editAddress = async (req, res) => {
    try {
        ...
        if (!userId || !addressId) {
            return res.status(400).json({ success: false, message: 'User Id and Address Id required' });
        }
